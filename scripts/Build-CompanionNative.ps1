param([string]$OutputName,[switch]$ReadOnlyAudit,[switch]$AssetLoader,[switch]$Protection,[switch]$Simulation,[switch]$Gaze)
...
if($Gaze -and $OutputName -notmatch '^companion_gaze_v[1-9][0-9]*\.dll$'){throw 'The gaze helper must use its own versioned DLL name.'}
...
$taskSource=if($Gaze){'companion_gaze.c'}elseif($Simulation){'companion_simulation.c'}elseif($Protection){'companion_protection.c'}elseif($AssetLoader){'companion_asset_loader.c'}elseif($ReadOnlyAudit){'companion_audit.c'}else{'companion_native.c'}
