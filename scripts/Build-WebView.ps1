...
& $taskCompiler @taskResourceArgs /nologo /target:winexe /platform:x64 "/out:$taskOutput\$OutputName" /reference:System.Windows.Forms.dll /reference:System.Drawing.dll /reference:System.Web.Extensions.dll "/reference:$taskOutput\Microsoft.Web.WebView2.Core.dll" "/reference:$taskOutput\Microsoft.Web.WebView2.WinForms.dll" (Join-Path $taskRoot 'bridge\AssemblyInfo.cs') (Join-Path $taskRoot 'bridge\WebViewHost.cs') (Join-Path $taskRoot 'bridge\DialogueOverlay.cs') (Join-Path $taskRoot 'bridge\ComposerKeys.cs') (Join-Path $taskRoot 'bridge\ModKeyBindings.cs') (Join-Path $taskRoot 'bridge\CompanionControls.cs') (Join-Path $taskRoot 'bridge\CompanionPanel.cs') (Join-Path $taskRoot 'bridge\CompanionLoading.cs') (Join-Path $taskRoot 'bridge\HostLifetime.cs') (Join-Path $taskRoot 'bridge\SupportReport.cs')
