...
Use the **Complete** archive for a normal install, or extract both matching **Scripts** and **Runtime** archives to the game root. Scripts contains data, Lua, JavaScript and configuration without EXE or DLL files; Runtime contains the executables, DLLs, local runtime and licenses. The split archives are two halves of one install and neither works alone. Every archive uses the game-root-relative `Dawnwalker/Binaries/Win64/ue4ss/Mods/DawnwalkerConvai` path.
