...
'@
 ...
 foreach($taskPart in @('Scripts','Runtime')){
  ...
  foreach($taskFile in Get-ChildItem -LiteralPath (Join-Path $taskOutput 'Dawnwalker') -Recurse -File){
   ...
   $taskBinary=$taskFile.Extension -in @('.exe','.dll')
   $taskLicense=$taskRelative -match '[\\/]licenses[\\/]'
   if(($taskPart -eq 'Scripts' -and !$taskBinary) -or ($taskPart -eq 'Runtime' -and ($taskBinary -or $taskLicense))){
