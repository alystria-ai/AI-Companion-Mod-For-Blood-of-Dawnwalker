...
try{
 ...
 sent=performance.now();await p.start('Hello '+cfg.name+'. How do you feel about travelling with me? Please answer in one short sentence.', 'You are '+cfg.name+' in Vale Sangora, talking to Coen. This is a short conversation, not a quest update. Answer naturally in one short sentence, without stage directions. Do not invent completed quests.');
...
const server=http.createServer(async(req,res)=>{
 ...
 res.end(req.url==='/'?'<script type="module" src="/bench.js"></script>':req.url==='/config'?JSON.stringify({apiKey:config.apiKey,id:profile.id,name:profile.name,mode}):req.url==='/bench.js'?b.outputFiles[0].text:await readFile('bridge/public/reply-capture.js'));
