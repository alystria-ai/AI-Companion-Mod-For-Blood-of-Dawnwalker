param([string]$OutputName,[switch]$ReadOnlyAudit,[switch]$AssetLoader,[switch]$Protection)
$ErrorActionPreference='Stop'
$taskKindCount=@($ReadOnlyAudit,$AssetLoader,$Protection).Where({$_}).Count
if($taskKindCount -gt 1){throw 'Choose one helper kind'}
if(!$OutputName){$OutputName=if($Protection){'companion_protection_v2.dll'}elseif($AssetLoader){'companion_assets_v2.dll'}elseif($ReadOnlyAudit){'companion_audit.dll'}else{'companion_native_v9.dll'}}
...
$taskSource=if($Protection){'companion_protection.c'}elseif($AssetLoader){'companion_asset_loader.c'}elseif($ReadOnlyAudit){'companion_audit.c'}else{'companion_native.c'}
